...
function AlbumThumbnailPanel({ album }: { album: any }) {
  ...
  // 展開時に取得した枚数を優先、なければDB保存済みの枚数を表示
  const displayCount = data?.photoCount ?? album.photoCount ?? null;
    ...
    <div className="border border-green-200 rounded-md overflow-hidden bg-white">
      ...
      >
        <span className="flex items-center gap-2 min-w-0">
          ...
          {displayCount != null && (
            <span className="flex-shrink-0 inline-flex items-center gap-0.5 bg-green-100 text-green-700 text-xs font-medium px-1.5 py-0.5 rounded-full">
