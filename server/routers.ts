...
export const appRouter = router({
  ...
  demo: router({
    ...
    getRandomPhotoWithAnalysis: protectedProcedure
      .input(z.object({ postCategory: z.string().nullable().optional() }).optional())
      .mutation(async ({ input }) => {
        try {
          const googlePhotos = await import("./google-photos-service");
          const { photo, album } = await googlePhotos.getRandomConstructionPhoto(input?.postCategory);
    ...
    generate: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const platformGuidelines: Record<string, { length: string; hashtagCount: string }> = {
          instagram: { length: "300〜500文字程度（最大2200文字まで可）", hashtagCount: "20〜30個" },
          x: { length: "100〜140文字程度（最大280文字以内）", hashtagCount: "3〜5個" },
          threads: { length: "150〜250文字程度（最大500文字以内・厳守）", hashtagCount: "5〜10個" },
        };

        const guide = platformGuidelines[input.platform];

        const presidentSystemPrompt = getSystemPrompt("ハゼモト建設");
        const response = await invokeLLM({
          messages: [
            {
              role: "system",
              content: presidentSystemPrompt,
            },
            {
              role: "user",
  ...
  postBalance: router({
    ...
    getMonthlyBalance: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // 当月の下書き投稿を取得
        const drafts = await db.getPostDraftsByUserId(ctx.user.id);
        const monthlyDrafts = drafts.filter(d => {
          const created = new Date(d.createdAt);
          return created >= startDate && created <= endDate &&
            d.companyName === input.companyName;
        });
        ...
        const classifiedDrafts = monthlyDrafts.map(draft => {
          const text = `${draft.title || ""} ${draft.instagramContent || ""} ${draft.xContent || ""}`;
          ...
          return { ...draft, detectedType };
        });

        const total = monthlyDrafts.length;
    ...
    generateAdvice: protectedProcedure
      .input(z.object({
        ...
        total: z.number(),
        recommendedBalance: z.record(z.string(), z.number()),
      }))
      .mutation(async ({ input }) => {
        const monthLabel = `${input.year ?? new Date().getFullYear()}年${input.month ?? (new Date().getMonth() + 1)}月`;

        const balanceSummary = Object.entries(input.typeCounts)
          .map(([type, count]) => {
            const actual = input.total > 0 ? Math.round((count / input.total) * 100) : 0;
            const recommended = input.recommendedBalance[type] ?? 0;
            const diff = actual - recommended;
            return `・${type}：${count}件（${actual}%） ← 推奨${recommended}% （差分：${diff > 0 ? '+' : ''}${diff}%）`;
          })
          .join("\n");
        ...
        const response = await invokeLLM({
          messages: [
            {
              role: "system",
              content: `あなたはハゼモト建設のSNS投稿コンサルタントです。月間の投稿バランスを分析し、具体的で実践的なアドバイスを日本語で提供してください。

ハゼモト建設について：
- 福岡県北九州市の地元密着型工務店（創業60年以上）
- 高性能住宅・リフォームが強み
- 子ども食堂・就労支援など地域活動にも積極的
- SNSでブランド認知と顧客との関係構築を目指している

推奨投稿バランス：施工事例40%・地域活動20%・スタッフ紹介20%・社長コラム10%・季節イベント10%

アドバイスは以下の形式で返してください：
1. 今月の総評（1〜2文）
2. 不足しているカテゴリとその理由（具体的に）
3. 来月に向けた具体的な投稿アイデア（2〜3件）

文体は親しみやすく、実践的に。箇条書きは使わず、会話的な文章で。`,
            },
            {
              role: "user",
              content: `${monthLabel}の投稿バランスを分析してアドバイスをください。

【今月の投稿実績（合計${input.total}件）】
${balanceSummary}

不足しているカテゴリを特定し、来月に向けた具体的なアドバイスをお願いします。`,
            },
          ],
        });
...
以下のJSON形式で返してください（説明文は不要、JSONのみ）：
{
  ...
  // ワンクリック投稿文修正ルーター
  refinePost: router({
    // Instagram投稿文をThreads用に500文字以内に短縮
    shortenForThreads: protectedProcedure
      .input(z.object({
      ...
      .mutation(async ({ input }) => {
        const { invokeLLM } = await import("./_core/llm");
        const systemPrompt = getSystemPrompt("\u30cf\u30bc\u30e2\u30c8\u5efa\u8a2d");
        const response = await invokeLLM({
          messages: [
            {
              role: "system",
              content: systemPrompt + "\n\nThreads\u5411\u3051\u306b500\u6587\u5b57\u4ee5\u5185\u306b\u77ed\u7e2e\u3059\u308b\u30a8\u30ad\u30b9\u30d1\u30fc\u30c8\u3067\u3059\u3002\u8981\u70b9\u3068\u60c5\u611f\u3092\u6b8b\u3057\u3001\u4e0d\u8981\u306a\u8aac\u660e\u3092\u524a\u308a\u3001Threads\u3089\u3057\u3044\u4f1a\u8a71\u7684\u306a\u6587\u4f53\u306b\u3002",
            },
            {
              role: "user",
              content: `\u4ee5\u4e0b\u306eInstagram\u6295\u7a3f\u6587\u3092Threads\u7528\u306b500\u6587\u5b57\u4ee5\u5185\u306b\u77ed\u7e2e\u3057\u3066\u304f\u3060\u3055\u3044\u3002\n\n\u91cd\u8981\u306a\u6307\u793a:\n1. 500\u6587\u5b57\u4ee5\u5185\u306b\u53b3\u5b88\u3059\u308b\u3053\u3068\n2. \u793e\u9577\u306e\u8a9e\u308a\u53e3\u3084\u6e29\u304b\u3044\u30c8\u30fc\u30f3\u3092\u6b8b\u3059\u3053\u3068\n3. \u6700\u3082\u4f1a\u8a71\u7684\u3067\u89aa\u3057\u307f\u3084\u3059\u3044\u90e8\u5206\u3092\u6b8b\u3059\u3053\u3068\n4. \u30cf\u30c3\u30b7\u30e5\u30bf\u30b0\u306f5\u301c10\u500b\u7a0b\u5ea6\u306b\u7d5e\u308b\u3053\u3068\n5. \u4fee\u6b63\u5f8c\u306e\u6295\u7a3f\u6587\u306e\u307f\u3092\u8fd4\u3059\u3053\u3068\uff08\u8aac\u660e\u4e0d\u8981\uff09\n\n\u5143\u306eInstagram\u6295\u7a3f\u6587:\n${input.originalText}`,
            },
          ],
        });
        const rawContent = response.choices[0].message.content;
        const shortenedText = typeof rawContent === "string" ? rawContent.trim() : input.originalText;
    ...
    refine: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // 「社長っぽく」の場合はcompanyProfiles.tsのsystemPromptを使用
        const refineSystemPrompt = input.refineType === 'president_style'
          ? getSystemPrompt("ハゼモト建設") + `\n\n${input.platform}向けの投稿文を修正するエキスパートです。修正後の投稿文は${maxLength}文字以内に収めてください。`
          : `あなたはハゼモト建設の${input.platform}向け投稿文を修正するエキスパートです。修正後の投稿文は${maxLength}文字以内に収めてください。`;
...
7. GBP投稿の文字数制限（1500文字）を意識し、400〜800文字程度にまとめる`;
    ...
    /** イベント告知専用AI投稿文生成 */
    generateEventPost: protectedProcedure
      .input(z.object({
        locationName: z.string(),
        eventTemplate: z.enum(['completion_tour', 'structure_tour', 'consultation', 'local_event', 'campaign_event', 'custom']),
        eventTitle: z.string().optional(),
        eventDate: z.string(),
        ...
        additionalInfo: z.string().optional(),
        generateReminder: z.boolean().optional(),
      }))
      .mutation(async ({ input }) => {
        const { invokeLLM } = await import('./_core/llm');
        const companyContext = input.locationName.includes('ハゼモト') ? `
...
- 創業60年以上の地域密着型工務店
- 強み：狭小地・擁壁土地・コンパクトハウスの実績、自前の多能工職人、高性能住宅・省エネ設計
- 地域活動：L'Atelier de Ash（パン屋）、未来のとびら（就労支援B型）、子ども食堂` : `【会社名】${input.locationName}`;
        ...
        const title = input.eventTitle || templateNames[input.eventTemplate] || 'イベント';

        const eventDetails = [
          `イベント名：${title}`,
          `開催日：${input.eventDate}`,
          input.eventTime ? `時間：${input.eventTime}` : null,
          input.venue ? `場所：${input.venue}` : null,
          input.capacity ? `定員：${input.capacity}` : null,
          input.applyUrl ? `申込URL：${input.applyUrl}` : null,
          input.contactPhone ? `問い合わせ：${input.contactPhone}` : null,
          input.staffName ? `担当者：${input.staffName}` : null,
          input.additionalInfo ? `補足：${input.additionalInfo}` : null,
        ].filter(Boolean).join('\n');

        const templateInstructions: Record<string, string> = {
          completion_tour: `「完成見学会」の告知投稿文を作成してください。実際に完成した住まいを体感できる価値（カタログでは分からない本物の空気感・広さ・素材感）、ハゼモト建設の強み（狭小地対応・高性能住宅・光熱費削減）を自然に盛り込み、「予約制・先着○組限定」などの希少性と参加メリット（専門家に直接相談できる・実際の暮らしをイメージできる）を含めてください。温かみのある語り口で、硬くなりすぎないように。`,
          structure_tour: `「構造見学会」の告知投稿文を作成してください。普段は見られない「家の骨格・断熱材・気密施工」を直接確認できる価値、「安心して長く住める家」への信頼感の訴求、技術力・施工品質への自信を含めてください。参加者へのメリット（品質を目で確認できる・疑問を直接聞ける）も明記してください。`,
          consultation: `「家づくり相談会」の告知投稿文を作成してください。「まだ漠然としている方でも大丈夫」という安心感、土地探し・資金計画・間取り相談など何でも聞ける雰囲気、押し売りなし・気軽に参加できることを強調し、担当者の親しみやすさも伝えてください。`,
          local_event: `「地域イベント」の告知投稿文を作成してください。地域への貢献・つながりへの想い、創業60年以上の地域密着の姿勢、参加しやすい雰囲気を温かみのある語り口で伝えてください。`,
          campaign_event: `「キャンペーン・特典イベント」の告知投稿文を作成してください。キャンペーン内容・特典の具体的な説明、期間・条件の明確化、お客様へのメリット、申込・問い合わせへの導線を含めてください。`,
          custom: `イベント告知の投稿文を作成してください。イベントの魅力・参加するメリット、開催詳細、申込・問い合わせへの導線を含めてください。`,
        };

        const instruction = templateInstructions[input.eventTemplate] || templateInstructions.custom;
        const systemPrompt = `あなたはGoogleビジネスプロフィール（GBP）のイベント告知投稿文作成の専門家です。
Webコンサルタント永友一郎氏のメソッドに基づき、以下の原則で投稿文を作成します：
1. お客様目線で書く（参加するメリットを前面に）
2. 具体的な情報を使う（日時・場所・定員）
3. 希少性・緊急性を自然に盛り込む（「先着○組」「○日まで」）
4. 温かみのある語り口で、書いている人の顔が見えるような文体
5. 専門用語を避け、参加したくなる内容にする
6. 末尾には必ず申込・問い合わせへの行動を促すCTAを入れる
7. GBP投稿の文字数制限（1500文字）を意識し、400〜700文字程度にまとめる`;
        ...
        let reminderContent3days: string | null = null;
        let reminderContent1day: string | null = null;

        if (input.generateReminder) {
          const r3 = await invokeLLM({
            messages: [
              { role: 'system', content: systemPrompt },
              { role: 'user', content: `${companyContext}\n\n上記イベントの「3日前リマインド投稿文」を作成してください。「あと3日！」という緊急感を出しつつ、まだ申し込んでいない方への背中を押す内容にしてください。200〜400文字程度。\n\n【イベント詳細】\n${eventDetails}` },
            ],
          });
          const r3Content = r3.choices?.[0]?.message?.content;
          reminderContent3days = typeof r3Content === 'string' ? r3Content : null;

          const r1 = await invokeLLM({
            messages: [
              { role: 'system', content: systemPrompt },
              { role: 'user', content: `${companyContext}\n\n上記イベントの「前日リマインド投稿文」を作成してください。「いよいよ明日！」という期待感を高める内容にしてください。参加予定者への確認事項（持ち物・アクセス等）も含めてください。200〜400文字程度。\n\n【イベント詳細】\n${eventDetails}` },
            ],
          });
          const r1Content = r1.choices?.[0]?.message?.content;
          reminderContent1day = typeof r1Content === 'string' ? r1Content : null;
        }

        return { mainContent, reminderContent3days, reminderContent1day };
      }),
  ...
  googlePhotoAlbums: router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        postCategory: z.string().max(64).nullable().optional(),
      }))
      .mutation(async ({ input }) => {
        return await db.createGooglePhotoAlbum({
          ...
          postCategory: input.postCategory ?? null,
        });
      }),
    /** アルバムを更新する */
    update: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const { id, targetSnsAccountIds, postCategory, ...rest } = input;
        ...
        if (postCategory !== undefined) {
          updates.postCategory = postCategory;
        }
    ...
    preview: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          ...
          if (!res.ok) return { thumbnails: [], photoCount: null as number | null };
          ...
          // 写真枚数を抽出（Googleフォト共有アルバムのHTMLから枚数テキストを探す）
          let photoCount: number | null = null;
          // パターン1: "123枚の写真" や "123 photos" などのテキスト
          const countMatch1 = html.match(/(\d+)\s*(?:枚の写真|photos?|items?)/i);
          if (countMatch1) {
            photoCount = parseInt(countMatch1[1], 10);
          }
          // パターン2: JSON埋め込みデータから枚数を抽出
          if (photoCount === null) {
            const jsonMatch = html.match(/"mediaItemsCount":\s*(\d+)/);
            if (jsonMatch) photoCount = parseInt(jsonMatch[1], 10);
          }
          // パターン3: og:descriptionから枚数を抽出
          if (photoCount === null) {
            ...
            if (descMatch) {
              const numInDesc = descMatch[1].match(/(\d+)/);
              if (numInDesc) photoCount = parseInt(numInDesc[1], 10);
            }
          }
          // photoCountをDBに保存（次回からリスト一覧で表示できるように）
          if (photoCount !== null) {
            try {
              const album = await db.getGooglePhotoAlbumByUrl(input.url);
              if (album) {
                await db.updateGooglePhotoAlbum(album.id, { photoCount });
              }
            } catch {
              // 保存失敗しても結果は返す
            }
          }
          return { thumbnails: ogImages.slice(0, 6), photoCount };
        } catch {
          return { thumbnails: [], photoCount: null as number | null };
        }
      }),
  }),
  ...
  hpLinks: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      const { hpLinkSettings } = await import('../drizzle/schema');
      const result = await dbInstance.select().from(hpLinkSettings).where(eq(hpLinkSettings.userId, ctx.user.id)).limit(1);
      return result[0] ?? null;
    }),

    save: protectedProcedure
      .input(z.object({
        constructionCasesUrl: z.string().optional(),
        visitReservationUrl: z.string().optional(),
        catalogRequestUrl: z.string().optional(),
        googleReviewUrl: z.string().optional(),
        modelHouseUrl: z.string().optional(),
        companyTopUrl: z.string().optional(),
        enableInstagram: z.boolean().optional(),
        enableX: z.boolean().optional(),
        enableThreads: z.boolean().optional(),
        enableGbp: z.boolean().optional(),
      }))
      .mutation(async ({ ctx, input }) => {
        const dbInstance = await db.getDb();
        if (!dbInstance) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'DBに接続できません' });
        const { hpLinkSettings } = await import('../drizzle/schema');
        const existing = await dbInstance.select().from(hpLinkSettings).where(eq(hpLinkSettings.userId, ctx.user.id)).limit(1);
        if (existing.length > 0) {
          await dbInstance.update(hpLinkSettings).set({ ...input }).where(eq(hpLinkSettings.userId, ctx.user.id));
        } else {
